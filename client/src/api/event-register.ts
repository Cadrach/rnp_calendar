...
const makeHook = <T>(fn: (eventId: number, data: RegisterBody) => Promise<T>) =>
  <TError = ErrorType<unknown>, TContext = unknown>(
    options?: { mutation?: UseMutationOptions<Awaited<T>, TError, Variables, TContext> },
  ): UseMutationResult<Awaited<T>, TError, Variables, TContext> => {
    const mutationFn: MutationFunction<Awaited<T>, Variables> = ({ event, data }) => fn(event, data);
